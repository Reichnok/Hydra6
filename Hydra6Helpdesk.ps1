...
#                                                                     #
#                                                                     #
# Version 6.00 (01.07.2022)                                           #
#     Perform checkpoints deletion based in elderness                 #
#     Perform checkpoints deletion based on combo multiselection box  #
#     Citrix Machine Catalog Update Remotely through Kanguro servers  #
#     Icon Shortcut redirected to local Kanguro Installation Path     #
#                                                                     #
#                                                                     #
#                                                                     #
#                                                               MAT   #
...
$form.Text = 'Select ADgroup you want to add to this user'
...
$listBox.Height = 80
<#
[void] $listBox.items.add('Hydra_ING')
[void] $listBox.items.add('Hydra_ING_Active_directory')
[void] $listBox.items.add('Hydra_ING_Common_Tasks')
[void] $listBox.items.add('Hydra_ING_SCCM')
[void] $listBox.items.add('Hydra_ING_Virtualization')
[void] $listBox.items.add('Hydra_ING_Helpdesk')
...
$result = $form.ShowDialog()
   ...
   if($x -eq "Hydra_ING"){$group=".\settings\Hydra_ING.lst"}
   if($x -eq "Hydra_ING_Active_directory"){$group=".\settings\Hydra_ING_Active_directory.lst"}
   if($x -eq "Hydra_ING_Common_Tasks"){$group=".\settings\Hydra_ING_Common_Tasks.lst"}
   if($x -eq "Hydra_ING_SCCM"){$group=".\settings\Hydra_ING_SCCM.lst"}
   if($x -eq "Hydra_ING_Virtualization"){$group=".\settings\Hydra_ING_Virtualization.lst"}
   if($x -eq "Hydra_ING_HELPDESK"){$group=".\settings\Hydra_ING_helpdesk.lst"}
   #>
   $group = ".\settings\Hydra_ING_helpdesk.lst"
   $SequencesListParam = $group
   get-childitem .\settings\*.lst |select name|select-object -Unique
